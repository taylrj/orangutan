...
const animate = ({
  ...
  const viewportHeight = frame.height - (image.marginTop + image.marginBottom)
  ...
  if (captionRef.current) {
    captionRef.current.setAttribute(
      ...
      transform: translate(${translateX * scale +
        (width + scale * width) / 2 +
        image.marginRight}px, ${translateY * scale +
        (height + scale * height) / 2 -
        (captionHeight + caption.offset)}px);
      margin-right: ${caption.marginRight}px;
    `
    )
  }
}
